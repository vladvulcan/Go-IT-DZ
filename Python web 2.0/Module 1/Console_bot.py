...
class Birthday(Field):
    ...
    def value(self, birthday):
        try:
            ...
            raise ValueError(
                "Invalid birthdate format. The date needs to be in YYYY-mm-dd format"
            )
...
class Record:
    ...
    def days_to_birthday(self):
        ...
        self.next_bd = datetime(
            next_birthday_year, self.birthday.value.month, self.birthday.value.day
        ).date()
...
def read_from_backup():
    backup_file = (
        r"C:\Users\User\Documents\Python\Go-IT-DZ\Python web 2.0\Module 1\backup.bin"
    )
...
def clear_user(command: str):
    name = command[1:]
    confirm = input(
        f"This will delete all the phones for existing user {name}. Proceed? (Y/N)\n"
    )
    if confirm.lower() == "y":
...
class Printer(AbstractClass):
    def console_output(self, object):
        print(object)
